...
/**
 * Service to test AWS account connections and validate credentials. Uses cached credentials via
 * AwsClientFactory for efficiency.
 */
@Service
...
public class AwsConnectionTester {
  ...
  /**
   * Tests the connection to an AWS account and returns connection details. This validates that: 1.
   * The role can be assumed (or access keys are valid) 2. Basic API calls work (STS
   * GetCallerIdentity) 3. EC2 DescribeRegions works (required for scanning)
   * <p>
   * Uses cached credentials via AwsClientFactory.resolveCredentials().
   */
  public ConnectionTestResult testConnection(AwsAccount account) {
    ...
    try {
      ...
      // Resolve credentials (uses cache if available, otherwise assumes role/decrypts keys)
      var credentials = clientFactory.resolveCredentials(account);

      // Create STS client with the credentials to test identity
      ...
      log.error("Connection test failed for account {}: {}",
          account.getAccountId(), e.getMessage(), e);

      // Evict credentials on error (they might be expired/invalid)
      clientFactory.evictCredentials(account.getId());
    }
